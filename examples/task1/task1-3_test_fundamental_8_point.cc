...
FundamentalMatrix fundamental_8_point (math::Matrix<double, 3, 8> const& points1
    ...
    Eigen::Matrix<double, 8, 9> A_eigen;
    for(int k=0; k<A_eigen.rows(); k++){
        double x1 = points1(0, k);
        double y1 = points1(1, k);
        double x2 = points2(0, k);
        double y2 = points2(1, k);
        Eigen::VectorXd A_row(9);
        A_row<<x2*x1, y1*x2, x2, y2*x1, y2*y1, y2, x1, y1, 1;
        A_eigen.row(k) = A_row;
    }

    // Get single value
    Eigen::JacobiSVD<Eigen::MatrixXd> svd(A_eigen, Eigen::ComputeFullU|Eigen::ComputeFullV);
    Eigen::VectorXd S_eigen_v = svd.singularValues();
    Eigen::MatrixXd U_eigen = svd.matrixU();
    Eigen::MatrixXd V_eigen = svd.matrixV();
    Eigen::VectorXd delta(9);
    delta = V_eigen.col(8); 
    F_matrix<<delta[0], delta[1], delta[2],
              delta[3], delta[4], delta[5],
              delta[6], delta[7], delta[8];
    
    Eigen::JacobiSVD<Eigen::MatrixXd> svd_f(F_matrix, Eigen::ComputeFullU|Eigen::ComputeFullV);
    S_eigen_v = svd_f.singularValues();
    U_eigen = svd_f.matrixU();
    V_eigen = svd_f.matrixV();
    Eigen::Matrix3d S_eigen(Eigen::Matrix3d::Zero());
    S_eigen(0, 0) = S_eigen_v(0);
    S_eigen(1, 1) = S_eigen_v(1);
    F_matrix = U_eigen * S_eigen * V_eigen.transpose();

    FundamentalMatrix F;
    for(int x=0; x<F_matrix.rows();x++){
        for(int y=0; y<F_matrix.cols();y++){
            F(x,y) = F_matrix(x,y);
        }
    } 
